...
public class PostDAO extends DBConnect {
    ...
    public int countPendingPostsThisWeek() {
        int count = 0;
        String sql = "SELECT COUNT(*) FROM Post WHERE Status = 'Pending' AND CreatedAt >= DATEADD(DAY, -7, GETDATE())";
    ...
    public int countPendingPostsPreviousWeek() {
        int count = 0;
        String sql = "SELECT COUNT(*) FROM Post WHERE Status = 'Pending' AND CreatedAt >= DATEADD(DAY, -14, GETDATE()) AND CreatedAt < DATEADD(DAY, -7, GETDATE())";
        try (PreparedStatement ps = connection.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                count = rs.getInt(1);
            }
