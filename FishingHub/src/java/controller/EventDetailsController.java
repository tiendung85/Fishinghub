...
public class EventDetailsController extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EventDetailsController</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EventDetailsController at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        Timestamp now = new Timestamp(System.currentTimeMillis());
            ...
            if (now.before(e.getStartTime())) {
                e.setEventStatus("Đang mở đăng ký");
            } else if (now.after(e.getEndTime())) {
                e.setEventStatus("Sự kiện đã kết thúc");
            } else {
